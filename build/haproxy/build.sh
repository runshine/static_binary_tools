...
VERSION="3.3.0"
PACKAGE="haproxy"
apt-get -y install curl ca-certificates wget xz-utils libssl-dev libncurses-dev autoconf git libwrap0-dev libreadline-dev flex bison  liblua5.3-dev liblua5.4-dev libpcre2-dev libz-dev libzstd-dev
...
sed -i 's/-lssl -lcrypto/-lssl -lcrypto -lz -lzstd/g' Makefile
make -j 8 TARGET=linux-glibc CFLAGS=-static PREFIX="${INSTALL_DIR}" USE_OPENSSL=1 USE_QUIC=1 USE_QUIC_OPENSSL_COMPAT=1 USE_LUA=0 USE_PCRE2=1
