...
curl -L -o "${SOURCE_DIR}/${PACKAGE}-${VERSION}.${EXT}" "https://github.com/htop-dev/htop/releases/download/${VERSION}/${PACKAGE}-${VERSION}.${EXT}"
...
./configure --prefix=${INSTALL_DIR} --enable-unicode --enable-static
make install

strip_elf_files "$INSTALL_DIR/sbin"
strip_elf_files "$INSTALL_DIR/bin"
package_release_tar "${INSTALL_DIR}" tcpdump-linux-${ARCH}.tar.gz