...
func stopService(serviceName string) bool {
	...
	// 如果服务有停止命令，先尝试使用停止命令
	stopSuccess := false
	stopCmd := strings.TrimSpace(status.Service.StopCmd)
	if stopCmd != "" {
		logger.Printf("[%s] Using stop command: %s", serviceName, stopCmd)
...
func executeStopCommand(serviceName string, status *ServiceStatus) bool {
	stopCmd := strings.TrimSpace(status.Service.StopCmd)
	if stopCmd == "" {
		logger.Printf("[%s] No stop command configured", serviceName)
		return false
	}
...
// validateWorkspaceForUninstall 卸载前校验工作空间路径，避免误删根路径等高风险目录
func validateWorkspaceForUninstall(workspacePath string) error {
	...
	if cleaned == string(os.PathSeparator) {
		return fmt.Errorf("refuse to delete root path: %s", cleaned)
	}
	// 业务强校验：路径必须包含 sothothv2
	if !strings.Contains(strings.ToLower(cleaned), "sothothv2") {
		return fmt.Errorf("workspace path must contain 'sothothv2': %s", cleaned)
	}
	return nil
}
...
func performUninstall(workspacePath string) error {
	...
	if isShuttingDown {
		shutdownMutex.Unlock()
		return fmt.Errorf("shutdown/uninstall already in progress")
	}
	isShuttingDown = true
	shutdownMutex.Unlock()

	start := time.Now()
	logger.Println("========== Starting Agent Uninstall ==========")
	logger.Printf("Uninstall workspace target: %s", workspacePath)
